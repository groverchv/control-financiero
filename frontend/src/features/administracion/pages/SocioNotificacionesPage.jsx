...
export const SocioNotificacionesPage = () => {
  ...
  const fetchNotificaciones = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        "[SocioNotificaciones] Error al cargar notificaciones:",
        err,
      );
    } finally {
      setLoading(false);
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      {!loading && (
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-none p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-500/20 flex items-center justify-center">
              <Bell className="h-5 w-5 text-emerald-600 dark:text-emerald-400" />
            </div>
            <div>
              <p className="text-2xl font-bold text-slate-900 dark:text-white">{noLeidas}</p>
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">No leídas</p>
            </div>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-none p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-slate-900 dark:text-white">{leidas}</p>
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Leídas</p>
            </div>
          </div>
        </div>
      )}
      ...
      {!loading && notificaciones.length > 0 && (
        <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-none flex flex-col sm:flex-row gap-4 justify-between items-center">
          <div className="relative w-full sm:max-w-md">
            <input
              ...
              className="w-full pl-10 pr-4 py-2 text-sm bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all placeholder:text-slate-400 text-slate-900 dark:text-white"
            />
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-none">
            <div className="divide-y divide-slate-100 dark:divide-slate-700/50">
              {paginatedNotificaciones.map((notif) => (
                <div
                  key={notif.id}
                  className={`notif-item p-5 flex gap-4 transition-colors hover:bg-slate-50 ${
                    notif.estado !== "leida" ? "notif-unread bg-emerald-50/30" : "notif-read"
                  }`}
                >
                  <div
                  ...
                  >
                    {notif.titulo.toLowerCase().includes("pago") ||
                    notif.titulo.toLowerCase().includes("cuota") ? (
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-1">
                      <h4
                        className={`text-sm font-bold ${notif.estado !== "leida" ? "text-slate-900 dark:text-white" : "text-slate-700 dark:text-slate-300"}`}
                      >
                        {notif.titulo}
                      </h4>
                    ...
                    <p className={`text-sm leading-relaxed mb-3 ${notif.estado !== "leida" ? "text-slate-600 dark:text-slate-300" : "text-slate-500 dark:text-slate-400"}`}>
                      {notif.descripcion}
                    </p>
