...
export const GestionMiembrosPage = () => {
  ...
  const rows = paginatedMiembros.map((miembro) => ({
    ...
    telefono: (() => {
      const raw = (miembro.telefono || '').trim();
      ...
      return (
        <div className="flex items-center gap-2">
          <span className="text-slate-600 font-mono text-xs">
            <HighlightMatch text={raw || '-'} query={searchTerm} />
          </span>
          {waUrl && (
            <a
              href={waUrl}
              ...
              title={`Iniciar chat de WhatsApp con ${miembro.nombre || 'socio'}`}
              aria-label={`WhatsApp a ${miembro.telefono}`}
            >
              ...
              <span>
                ...
                {editingMember ? (
                  ...
                  <>
                    ...
                    <strong className="text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded border border-amber-200 flex items-center gap-1 mt-2 text-xs">
                      <AlertTriangle className="h-3.5 w-3.5 inline text-amber-700 shrink-0" />
                      Se le generará automáticamente una deuda de {formData.monto_inscripcion ?? 150} Bs. por motivo de inscripción.
                    </strong>
                  </>
                )}
              </span>
