...
DECLARE
    ...
    v_estado_cuota     text;
BEGIN
    -- Primer usuario → admin y monto inscripción 0; el resto → rol y monto del metadata o defecto
    SELECT count(*) INTO v_count FROM public.miembro;
    IF v_count = 0 THEN
        v_rol := 'admin';
        v_monto_inscripcion := 0;
    ...
    ELSE
        ...
        v_estado_cuota := CASE WHEN v_monto_inscripcion = 0 THEN 'pagado' ELSE 'pendiente' END;
    END IF;
    ...
    VALUES (
        new.id,
        CASE
            WHEN v_count = 0 THEN 'Inscripción Gratuita (Primer Usuario)'
    ...
    VALUES (
        new.id,
        '¡Bienvenido!',
        'Tu cuenta ha sido creada. ¡Te damos una cordial bienvenida a la Asociación de Profesionales Financieros de Santa Cruz!'
    );
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_tables
        WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'ingreso'
    ) THEN
        ALTER PUBLICATION supabase_realtime ADD TABLE public.ingreso;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_tables
        WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'egreso'
    ) THEN
        ALTER PUBLICATION supabase_realtime ADD TABLE public.egreso;
    END IF;
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_tables
        WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'activos'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_tables
        WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'jurado'
    ) THEN
        ALTER PUBLICATION supabase_realtime ADD TABLE public.jurado;
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_tables
        WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'inscripcion'
    ) THEN
        ALTER PUBLICATION supabase_realtime ADD TABLE public.inscripcion;
    END IF;
END $$;
