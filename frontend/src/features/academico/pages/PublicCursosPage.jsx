...
export const PublicCursosPage = () => {
          ...
          .filter(a => {
            ...
            if (a.fecha && a.hora) {
              ...
              if (!isNaN(courseStart.getTime())) {
                const now = new Date();
                const hideTime = new Date(courseStart.getTime() + 2 * 60 * 60 * 1000); // 2 hours after start (1 hour after finalizing)
  ...
  useEffect(() => {
    ...
    const reevaluateStates = () => {
      setActividades(prev => {
        ...
        const hasChanges = updated.length !== prev.length || updated.some((a, idx) => a.estado !== prev[idx].estado);
  ...
  const filteredActividades = actividades.filter(act => {
    ...
    return (
      ...
      (act.fecha && new Date(act.fecha + 'T00:00:00').toLocaleDateString('es-ES').includes(query))
    );
  });
            ...
            <Link key={act.id} to={`/actividades/${act.id}`} className="group relative flex flex-col rounded-2xl sm:rounded-3xl bg-white border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden hover:-translate-y-2">
               ...
               <div className="relative h-44 sm:h-56 w-full overflow-hidden">
                 ...
                 <div className="absolute top-4 left-4">
                   <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider shadow-lg bg-emerald-500/90 text-white backdrop-blur-md border border-white/10">
                     {act.tipo_nombre}
                   </span>
                 </div>
                 {/* Badge de Estado */}
                 <div className="absolute top-4 right-4">
                   <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg backdrop-blur-md border border-white/20 text-white ${
                     act.estado === 'finalizado' || act.estado === 'finalizada' ? 'bg-slate-900/60' : 'bg-emerald-500/80'
                   }`}>
                     {act.estado === 'en_curso' ? 'En proceso' : act.estado === 'finalizado' ? 'Finalizado' : 'Programada'}
                   </span>
                 </div>
               </div>
                 ...
                 <div className="mt-6 pt-6 border-t border-slate-50 grid grid-cols-3 gap-2">
                   <div className="flex items-center gap-1.5 text-slate-600">
                     ...
                     <div className="flex flex-col min-w-0">
                       <span className="text-[9px] uppercase font-bold text-slate-400 leading-none">FECHA</span>
                       <span className="text-xs font-bold truncate">{new Date(act.fecha + 'T00:00:00').toLocaleDateString('es-ES', { day: '2-digit', month: 'short' })}</span>
                     </div>
                   </div>
                   <div className="flex items-center gap-1.5 text-slate-600">
                     <Clock className="h-4 w-4 text-emerald-500 shrink-0" />
                     <div className="flex flex-col min-w-0">
                       <span className="text-[9px] uppercase font-bold text-slate-400 leading-none">HORARIO</span>
                       <span className="text-xs font-bold truncate">{act.hora ? act.hora.substring(0, 5) : '--:--'}</span>
                     </div>
                   </div>
                   <div className="flex items-center gap-1.5 text-slate-600">
                     ...
                     <div className="flex flex-col min-w-0">
                       ...
                       <span className="text-xs font-bold truncate">{act.cupos || 0} plazas</span>
                     </div>
                   </div>
                 </div>
